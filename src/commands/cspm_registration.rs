...
pub enum Action {
    /// Query CSPM Registration policy setting IDs
    List {
    ...
    /// Get CSPM Registration policy setting details
    Get {
        /// Policy setting ID(s)
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/cspm/queries/policy-settings/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!("/cspm/entities/policy-settings/v1?{}", ids.join("&"));
            client.get(&path).await
        }
    }
}
