...
pub enum Action {
    /// Query data protection configuration IDs
    ///
    /// Response fields:
    ///   resources  - array of configuration ID strings
    ...
    /// Get data protection configuration details by ID
    ///
    /// Response fields:
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/data-protection/queries/configurations/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!(
                "/data-protection/entities/configurations/v1?{}",
                ids.join("&")
            );
            client.get(&path).await
        }
