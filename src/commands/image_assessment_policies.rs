...
pub enum Action {
    /// List image assessment policy IDs
    ///
    /// Response fields:
    ...
    /// Get image assessment policy details by ID
    ///
    /// Response fields:
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/image-assessment-policies/queries/policies/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!(
                "/image-assessment-policies/entities/policies/v1?{}",
                ids.join("&")
            );
            client.get(&path).await
        }
