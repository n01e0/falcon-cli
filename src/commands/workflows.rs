...
pub enum Action {
    /// Query workflow definition IDs
    ///
    /// Response fields:
    ///   resources  - array of workflow definition ID strings
    ...
    ///
    /// Response fields:
    ///   id         - workflow definition identifier
    ///   name       - workflow name
    ///   description - workflow description
    ///   enabled    - whether the workflow is enabled
    Get {
        /// Workflow definition ID(s)
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/workflows/queries/definitions/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!("/workflows/entities/definitions/v1?{}", ids.join("&"));
            client.get(&path).await
        }
    }
}
