...
pub enum Action {
    /// Query case activity IDs
    ///
    /// Response fields:
    ///   resources  - array of case activity ID strings
    ...
    /// Get case activity details by ID
    ///
    /// Response fields:
    ///   id         - case activity identifier
...
pub async fn execute(client: &FalconClient, action: Action) -> Result<serde_json::Value> {
    match action {
        ...
        Action::Get { id } => {
            ...
            let path = format!(
                "/message-center/entities/case-activities/v1?{}",
                ids.join("&")
            );
            client.get(&path).await
        }
