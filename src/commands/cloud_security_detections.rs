...
pub enum Action {
    /// Query Cloud Security Detections alert IDs
    List {
    ...
    /// Get Cloud Security Detections alert details
    Get {
        /// Alert ID(s)
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/cloud-security-detections/queries/alerts/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!(
                "/cloud-security-detections/entities/alerts/v1?{}",
                ids.join("&")
            );
            client.get(&path).await
        }
