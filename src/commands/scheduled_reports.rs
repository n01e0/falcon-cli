...
pub enum Action {
    ...
    ///
    /// Response fields:
    ///   resources  - array of scheduled report ID strings
    ...
    ///
    /// Response fields:
    ///   id         - scheduled report identifier
    ///   name       - report name
    ///   status     - report status
    ///   schedule   - report schedule configuration
    Get {
        /// Scheduled report ID(s)
...
pub async fn execute(client: &FalconClient, action: Action) -> Result<serde_json::Value> {
    match action {
        ...
        Action::Get { id } => {
            ...
            let path = format!("/reports/entities/scheduled-reports/v1?{}", ids.join("&"));
            client.get(&path).await
        }
    }
}
