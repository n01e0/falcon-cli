...
pub enum Action {
    /// Query D4C Registration account IDs
    List {
    ...
    /// Get D4C Registration account details
    Get {
        /// Account ID(s)
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/d4c-registration/queries/accounts/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!("/d4c-registration/entities/accounts/v1?{}", ids.join("&"));
            client.get(&path).await
        }
    }
}
