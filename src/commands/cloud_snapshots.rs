...
pub enum Action {
    /// Query Cloud Snapshots deployment IDs
    List {
    ...
    /// Get Cloud Snapshots deployment details
    Get {
        /// Deployment ID(s)
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/snapshots/queries/deployments/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!("/snapshots/entities/deployments/v1?{}", ids.join("&"));
            client.get(&path).await
        }
    }
}
