...
pub enum Action {
    /// Query Cloud Security resource IDs
    List {
    ...
    /// Get Cloud Security resource details
    Get {
        /// Resource ID(s)
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/cloud-security/queries/resources/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!("/cloud-security/entities/resources/v1?{}", ids.join("&"));
            client.get(&path).await
        }
    }
}
