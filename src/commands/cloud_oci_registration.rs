...
pub enum Action {
    /// Query Cloud OCI Registration account IDs
    List {
    ...
    /// Get Cloud OCI Registration account details
    Get {
        /// Account ID(s)
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/cloud-connect-oci/queries/account/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!("/cloud-connect-oci/entities/account/v1?{}", ids.join("&"));
            client.get(&path).await
        }
    }
}
