...
pub enum Action {
    /// Query custom storage object IDs
    List {
    ...
    /// Get custom storage object details
    Get {
        /// Custom storage object ID(s)
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/custom-storage/queries/objects/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!("/custom-storage/entities/objects/v1?{}", ids.join("&"));
            client.get(&path).await
        }
    }
}
