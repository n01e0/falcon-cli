...
pub enum Action {
    /// Query device content policy IDs
    ///
    /// Response fields:
    ...
    /// Get device content policy details by ID
    ///
    /// Response fields:
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/device-content/queries/policies/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!("/device-content/entities/policies/v1?{}", ids.join("&"));
            client.get(&path).await
        }
    }
}
