...
pub enum Action {
    /// List Spotlight evaluation logic IDs
    ///
    /// Response fields:
    ...
    /// Get Spotlight evaluation logic details by ID
    ///
    /// Response fields:
    ///   id                    - evaluation logic identifier
    ///   title                 - logic title
    ///   description           - logic description
    ///   logic_type            - type of evaluation logic
...
pub async fn execute(client: &FalconClient, action: Action) -> Result<serde_json::Value> {
    match action {
        ...
        Action::Get { id } => {
            ...
            let path = format!("/spotlight/entities/evaluation-logic/v1?{}", ids.join("&"));
            client.get(&path).await
        }
    }
}
