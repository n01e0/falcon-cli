use clap::Subcommand;

use crate::client::FalconClient;
use crate::commands::build_query_path;
use crate::error::Result;

#[derive(Subcommand, Debug)]
pub enum Action {
    /// List RTR admin put-file IDs
    ///
    /// Response fields:
    ///   resources  - array of put-file ID strings
    ///   errors     - array of error objects (if any)
    List {
        /// FQL filter expression
        #[arg(long)]
        filter: Option<String>,

        /// Maximum number of results
        #[arg(long, default_value = "100")]
        limit: u32,

        /// Pagination offset
        #[arg(long)]
        offset: Option<String>,
    },
    /// Get RTR admin put-file details by ID
    ///
    /// Response fields:
    ///   id                    - put-file identifier
    ///   name                  - file name
    ///   description           - file description
    ///   file_type             - file type
    ///   platform              - array of supported platforms
    ///   size                  - file size in bytes
    ///   created_timestamp     - file creation timestamp
    ///   modified_timestamp    - file modification timestamp
    Get {
        /// Put-file ID(s)
        #[arg(long, required = true, num_args = 1..)]
        id: Vec<String>,
    },
}

pub async fn execute(client: &FalconClient, action: Action) -> Result<serde_json::Value> {
    match action {
        Action::List {
            filter,
            limit,
            offset,
        } => {
            let path = build_query_path(
                "/real-time-response/queries/put-files/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
            client.get(&path).await
        }
        Action::Get { id } => {
            let ids: Vec<String> = id.iter().map(|i| format!("ids={}", i)).collect();
            let path = format!(
                "/real-time-response/entities/put-files/v2?{}",
                ids.join("&")
            );
            client.get(&path).await
        }
    }
}
