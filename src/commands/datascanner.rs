...
pub enum Action {
    ...
    /// Get DataScanner scan details
    Get {
        /// DataScanner scan ID(s)
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/data-security-dspm/queries/scans/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!("/data-security-dspm/entities/scans/v1?{}", ids.join("&"));
            client.get(&path).await
        }
    }
}
